...
rolling_forecast <- function(y_train, y_test, p_ar, q_ma, p_garch, q_garch) {
  ...
  for (i in 1:n_test) {
    ...
    # rolling_preds[i] <- sqrt(as.numeric(pred@forecast$sigmaFor[1]))
...
## Plot the test set
plot(time_index, rolling_preds, type = "l", col = "red", main = "Test Set vs. Rolling Predictions", xlab = "Time", ylab = "Value", 
     ylim = c(-0.04, 0.04))
...
legend("topleft", legend = c("Test Set", "Rolling Predictions"), col = c("black", "red"), lty = 1, bty = "n")
```
